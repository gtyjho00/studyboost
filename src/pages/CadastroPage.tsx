...
export function CadastroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await signUp(sanitize(email), password, sanitize(name));
    if (error) {
    ...
    <div className="min-h-screen bg-dark-900 flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-dark-300">Comece a aprender de graça agora</p>
        </div>

        <div className="card p-8">
          <button
            onClick={signInWithGoogle}
            className="w-full flex items-center justify-center gap-3 py-3 rounded-xl font-semibold text-white transition-all duration-200 bg-white/[0.06] border border-white/10 hover:bg-white/[0.1] hover:border-white/20 mb-6"
          >
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-dark-400 text-xs">ou</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
